...
namespace penguin_new.Perlin
{
    using Math = System.Math;
    public class NoiseMaker
    {
        // Adapted from Ken Perlin reference implementation
        ...
        public static float Noise(float x, float y, float z)
        {
            ...
            int A = p[X] + Y, AA = p[A] + Z, AB = p[A + 1] + Z,      
                B = p[X + 1] + Y, BA = p[B] + Z, BB = p[B + 1] + Z; // creates hash coordinates of the 8 cube corners


            return MathHelper.Lerp(
                    MathHelper.Lerp(
                         MathHelper.Lerp(Grad(p[AA], x, y, z),Grad(p[BA], x - 1, y, z),u)
                        ,
                        MathHelper.Lerp(Grad(p[AB], x, y - 1, z),Grad(p[BB], x - 1, y - 1, z),u),v
                    ),MathHelper.Lerp(
                        MathHelper.Lerp(
                            Grad(p[AA + 1], x, y, z - 1),Grad(p[BA + 1], x - 1, y, z - 1),u)
                        ,
                        MathHelper.Lerp(
                            Grad(p[AB + 1], x, y - 1, z - 1),Grad(p[BB + 1], x - 1, y - 1, z - 1),u)
                        ,v), w); 
        ...
        {
            switch(hash & 15) // hash as a 4 bit code
            {
                case 0: return  x + y;
                case 1: return -x + y;
                case 2: return  x - y;
